...
data_given_theta = [
    graph.trace("y[0] ~ normal(sigma=0.1)")(
        lambda theta: -(theta[0] - y[0])**2 / sigma**2 / 2),
    graph.trace("y[1] ~ normal(sigma=0.1)")(
        lambda theta: -(theta[0] - y[1])**2 / sigma**2 / 2),
]

integral = [
    graph.trace("theta ~ normal(sigma=1)")(graph.Integral(data_given_theta[0],
                                                          theta_given_psi[0],
                                                          draws=1000,
                                                          init=[0],
                                                          scale=[0.1],
                                                          log=True)),
    graph.trace("theta ~ normal(sigma=5)")(graph.Integral(data_given_theta[1],
                                                          theta_given_psi[1],
                                                          draws=1000,
                                                          init=[0],
                                                          scale=[0.1],
...
with open("analitical.trace.gv", "w") as file:
    graph.graphviz(file)
